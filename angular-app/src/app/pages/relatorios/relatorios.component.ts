...
export class RelatoriosComponent implements OnInit {
  ...
  carregarRelatorios() {
    ...
    this.apiService.getRelatorioVendasPorMarca().subscribe({
      next: (data: RelatoriosComponent[]) => (this.vendasPorMarca = data),
      error: (err: unknown) =>
        console.error("Erro ao carregar vendas por marca:", err),
    });

    this.apiService.getRelatorioProdutosMaisVendidos().subscribe({
      next: (data: RelatoriosComponent[]) => (this.produtosMaisVendidos = data),
      error: (err: unknown) =>
        console.error("Erro ao carregar produtos mais vendidos:", err),
    });

    this.apiService.getRelatorioRentabilidade().subscribe({
      next: (data: RelatoriosComponent[]) => {
